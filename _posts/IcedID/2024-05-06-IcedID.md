...
So, we will look at the exports but some of them doesn't make any thing too except `DllRegisterServer` it calls a lot API'S and functions

![alt text](dbg1.png)
After start debugging we will reach function called `VirtualAlloc` that it will be ready to be full with some thing suspicious so, we will go to the return function waiting the result.

![alt text](dbg3.png)
And here it is our memory dump is full with data starting with `MZ` so, we have a new `PE` lets start to make an analysis to it.
